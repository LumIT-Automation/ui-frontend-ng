...
import { certificatesFetch, certificateModifyError } from '../../../f5/store'
...
class Modify extends React.Component {
  ...
  certificateInstall =  async () => {
    ...
    let rest = new Rest(
      ...
      error => {
        this.props.dispatch(certificateModifyError(error))
        this.setState({loading: false, response: false})
      }
    )
    await rest.doXHR(`f5/${this.props.asset.id}/${this.props.partition}/certificate/${this.state.request.certificateName}/`, this.props.token, body )
  }
  ...
  render() {
    return (
      <React.Fragment>
        ...
        <Modal
          title={<p style={{textAlign: 'center'}}>Modify certificate</p>}
          centered
          destroyOnClose={true}
          ...
          <React.Fragment>
            <React.Fragment>
              <Row>
                ...
                <Col span={10}>
                  <p style={{marginRight: 10, marginTop: 5}}>{this.state.request.certificateName}</p>
                </Col>
              </Row>
              <br/>
            </React.Fragment>
        ...
        {this.state.visible ?
          <React.Fragment>
            { this.props.certificateModifyError ? <Error component={'certificates add'} error={[this.props.certificateModifyError]} visible={true} type={'certificateModifyError'} /> : null }
          </React.Fragment>
        :
          null
        }
...
export default connect((state) => ({
  ...
  certificateModifyError: state.f5.certificateModifyError,
}))(Modify);
