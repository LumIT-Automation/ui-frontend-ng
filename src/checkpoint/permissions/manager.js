...
class Manager extends React.Component {
  ...
  permissionsGet = async () => {
    ...
    await rest.doXHR("checkpoint/permissions/", this.props.token)
    return r
  }
  ...
  assetWithDetails = async (assets, permissions) => {
    ...
    try {
      Object.values(newPermissions).forEach((perm, i) => {
        const asset = assetsObject.find(a => a.id === perm.domain.asset_id)
        perm.asset = asset
      });
  ...
  render() {
    return (
      <React.Fragment>
        ...
        { this.props.permissionsError ? <Error component={'permissions manager checkpoint'} error={[this.props.permissionsError]} visible={true} type={'permissionsError'} /> : null }
      </React.Fragment>
    )
  }
}
...
export default connect((state) => ({
  ...
  permissionsFetch: state.checkpoint.permissionsFetch,

  assetsError: state.checkpoint.assetsError,
  identityGroupsError: state.checkpoint.identityGroupsError,
  permissionsError: state.checkpoint.permissionsError,
}))(Manager);
