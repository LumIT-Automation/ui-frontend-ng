...
class Add extends React.Component {
  ...
  newIdentityGroupAdd = async (identityGroupId, cn) => {
    ...
    await rest.doXHR(`checkpoint/identity-groups/`, this.props.token, b )
    return r
  }
  ...
  permissionAdd = async () => {
    ...
    await rest.doXHR(`checkpoint/permissions/`, this.props.token, b )
  }
  ...
  render() {
    return (
      <React.Fragment>
                      ...
                      { (this.state.domains && this.state.domains.length > 0) ?
                        <React.Fragment>
                          {this.state.errors.domainError ?
                            <Select
                              value={this.state.request && this.state.request.domain ? this.state.request.domain.name : null}
                              showSearch
        ...
        {this.state.visible ?
          <React.Fragment>
            { this.props.identityGroupsError ? <Error component={'permissionAdd checkpoint'} error={[this.props.identityGroupsError]} visible={true} type={'identityGroupsError'} /> : null }
            { this.props.rolesError ? <Error component={'permissionAdd checkpoint'} error={[this.props.rolesError]} visible={true} type={'rolesError'} /> : null }
            { this.props.domainsError ? <Error component={'permissionAdd checkpoint'} error={[this.props.domainsError]} visible={true} type={'domainsError'} /> : null }
            { this.props.newIdentityGroupAddError ? <Error component={'permissionAdd checkpoint'} error={[this.props.newIdentityGroupAddError]} visible={true} type={'newIdentityGroupAddError'} /> : null }
            { this.props.permissionAddError ? <Error component={'permissionAdd checkpoint'} error={[this.props.permissionAddError]} visible={true} type={'permissionAddError'} /> : null }
          </React.Fragment>
        :
          null
        }
...
export default connect((state) => ({
  ...
  newIdentityGroupAddError: state.checkpoint.newIdentityGroupAddError,
  domainsError: state.checkpoint.domainsError,
  permissionAddError: state.checkpoint.permissionAddError,
}))(Add);
